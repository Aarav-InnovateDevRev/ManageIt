...
# ── AI CHATBOT ROUTE (Grok / xAI example) ──
...
def chat():
    ...
    # Replace with your real xAI / Grok API key and endpoint
    api_key = os.environ.get("XAI_API_KEY")  # add this to Render env vars
    if not api_key:
        return jsonify({"error": "AI API key not set"}), 500

    try:
        response = requests.post(
            "https://api.x.ai/v1/chat/completions",  # example endpoint – check docs
            headers={
